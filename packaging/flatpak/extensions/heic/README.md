...
The main `io.github.yeagoo.imgconvert` Flatpak still sets
...
local Flatpak environment that can resolve the main `io.github.yeagoo.imgconvert`
runtime/app.

The real extension:

- use id `io.github.yeagoo.imgconvert.Codecs.Heic`;
- use branch `1`;
- builds `libde265` v1.1.1 and `libheif` v1.23.1 from pinned upstream source
  tarballs;
- installs a tiny `imgconvert-heic-helper` wrapper around `heif-dec`;
