...
- **WebP method 4**(method 6 几乎不省;2 大 ~5%;当前通过 `webp::WebPConfig` + `encode_advanced()` 传入,不需要额外引入 `libwebp-sys` 直连依赖)
...
- v1 校验规则:
  ...
  - `args` 是 argv 模板,占位符 `{input}` / `{output}` 必须各自作为独立 argv entry 出现;主程序不执行 shell。
...
- 第一版只做 `HEIC/HEIF -> PNG/RGBA/temp file` 再进入现有 core 管线;不提供 HEIC 编码输出,不承诺保留 HEIC 原始元数据。
- 安全约束:helper 路径必须来自受信任安装目录或用户显式选择;manifest 不允许任意 shell;传参使用 argv/JSON,禁止拼 shell 命令;临时文件放应用受控目录并清理。helper stdout 不落盘,stderr 只保留前 64 KiB;helper 输出 PNG 读取上限为 512 MiB。
...
- **P3 Flatpak 第一版**:`packaging/flatpak/com.ivmm.imgconvert.yml` 固定 app-id、desktop/metainfo、portal-friendly 权限和主包禁外部 codec helper(`IMGCONVERT_DISABLE_EXTERNAL_CODECS=1`)。当前 manifest 使用 Flathub 可解析且未 EOL 的 GNOME `50` runtime 与 `release:flatpak:prepare` 生成的 release archive,仍不得把 HEIC/helper 放进主包。
...
## 5. ICC / EXIF / XMP 保真(脏活,Hando 有带测试的实现可参考重写)

- **当前语义**:默认剥离 ICC/EXIF/XMP;用户开启 `preserveMetadata` 时才写回。解码阶段会尽量提取 metadata 到 `ImageData { icc, exif, xmp }`,编码阶段由 `EncodeOptions.preserve_metadata` 控制是否落容器。
- **JPEG**:已实现 APP1 EXIF、APP1 XMP 与 APP2 ICC 1-based 分块提取/写回。EXIF 存 TIFF payload,写 JPEG 时补 `Exif\0\0`;XMP 存 raw packet,写 JPEG 时补 Adobe XMP namespace 前缀。
- **PNG**:已实现 `iCCP` zlib 解压/压缩、`eXIf` 和未压缩 `iTXt` XMP(`XML:com.adobe.xmp`)提取/写回;写入发生在 **oxipng 优化之后**、`IHDR` 后,避免优化器剥离。
- **WebP**:已实现 RIFF chunk 手术;必要时插入/更新 `VP8X`,设置 ICC/EXIF/XMP flag,写 `ICCP`、`EXIF` 与 `XMP ` chunk 并更新 RIFF size。
- **AVIF**:✅ `libavif-sys` 路径已通过 `avifImageSetProfileICC` / `avifImageSetMetadataExif` 写回 ICC/EXIF;解码读取 `avifImage.icc/exif`。AVIF XMP 当前未接入,裸 `ravif`/`image` 被弃用的核心原因仍是容器元数据/ICC 控制弱。
...
- **XMP 边界**:当前只做 raw packet 透传,不解析 XML、不改写 XMP 内可能存在的 orientation/IPTC/编辑历史字段;JPEG extended XMP 分片与 PNG 压缩 `iTXt` XMP 暂不保留。JPEG 单 APP1 XMP 超过容器上限会明确返回编码错误。需要语义级编辑时再引入单独 metadata 模块与测试 corpus。
- **Display P3 / ICC 测试**:core 测试中自生成 Apache-2.0 兼容的 Display P3 ICC fixture,覆盖 P3 PNG 输入开启 `preserveMetadata` 后转换到 JPEG/PNG/WebP/AVIF 的端到端 ICC 逐字节保留。当前语义仍是**元数据保真**,不是像素级 ICC 色彩变换;真实色彩管理/线性空间 resize 仍留给后续 `8/16/float` 像素表示升级。
- **未做**:AVIF XMP、IPTC、EXIF MakerNote 语义解析、HEIC helper 原始元数据透传、像素级 ICC 色彩管理/线性化 resize。
...
- P2 已落地每格式质量下限阈值:JPEG/WebP/AVIF 支持 30-100,低于 30 视为禁用;自动质量搜索不得低于该下限。PNG 无 quality 搜索,WebP 无损模式忽略有损质量下限。
...
- **代际损失防护**:对已是有损的源(JPEG/AVIF/lossy WebP),按 **bits-per-pixel** 分级收紧门槛;当前最低收益阈值为 2%/3%/5%/8%。VP8L lossless WebP 不触发。PNG 默认仍按无损源处理,但若 core 的 JPEG 8×8 网格 hint 明显命中,会在用户启用 `generationLossProtection` 时按有损来源处理。
- **结果缓存**:Tauri 层用源文件 `blake3` + 目标格式 + 编码设置 hash 生成 cache key。缓存只记录已有输出的 hash/size,命中时跳过重新编码;不缓存图片内容。默认开启。
- 未做:AVIF/WebP 平台 benchmark、自动质量 wall-clock 超时/取消点细化、PNG 其他压缩噪声指纹。
...
- **P0.5 文件授权边界**:`src-tauri/src/access.rs` 已作为唯一授权路径 grant 入口,当前直发路径、剪贴板临时文件和输出目录都先经该层。该层刻意不要求 canonical 路径,避免破坏 Flatpak portal 映射路径;macOS 后续在同一层接 `startAccessingSecurityScopedResource`/`stop` 生命周期。
...
- **Windows 打包/Store 护栏第一批**:`src-tauri/tauri.windows.conf.json` 是 Tauri 在 Windows 上自动合并的直发配置,面向 `.msi`/NSIS。它显式禁止降级安装、使用 SHA-256 signing digest、silent embedded WebView2 bootstrapper、稳定 WiX `upgradeCode` 和 NSIS current-user 默认安装。`pnpm run release:windows:direct:check` 会校验这些边界;`IMGCONVERT_DISABLE_EXTERNAL_CODECS=1 pnpm run release:windows:store:check` 只做 Store candidate preflight,真实 MSIX/`runFullTrust`/Partner Center 仍需 Windows 阶段实测。
