...
| **Windows 可选插件** | ✅ decode-only | ❌ 不做 | 独立 `imgconvert-heic-helper.exe`;自建 decode-only libheif/libde265 动态包,不直接整包带 MSYS2 `libheif` 发行包(其依赖可能含 x265) |
...
- 安全约束:helper 路径必须来自受信任安装目录或用户显式选择;manifest 不允许任意 shell;传参使用 argv/JSON,禁止拼 shell 命令;临时文件放应用受控目录并清理。
- 分发约束:外部 helper 主要面向直发包/用户自行安装场景;App Store/MS Store/Flathub 构建默认禁用外部 helper,除非对应渠道明确允许并完成单独合规设计。
