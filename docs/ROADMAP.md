...
- [x] **发布 readiness 报告入口**:`pnpm run release:readiness` 默认按第一期 GitHub-only 范围汇总静态检查、Linux artifact、updater artifact 和外部阻塞项;`pnpm run release:readiness:github:ready` 是 GitHub Release 发布前硬闸门,任何 in-scope artifact/签名/updater 前置不是 ready 都会失败;`pnpm run release:readiness:all` / `--scope=all` 才显示 Flathub、macOS、Windows、商店和长期 benchmark/corpus 项。该入口不构建、不联网、不触发 paid runner,用于每次发布前快速判断“本地能继续什么 / 必须等什么”。
